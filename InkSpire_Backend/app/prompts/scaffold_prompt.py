...
def get_scaffold_prompt():
    return ChatPromptTemplate.from_messages([
        (
            ...
            "- Output must be valid JSON (double quotes for all keys/strings).\n"
            "- Do NOT include any explanation or extra text.\n"
            "- Do NOT include trailing commas.\n"
            "- Do NOT include unescaped newlines inside strings.\n"
            "- If a fragment/text contains a newline, replace it with a space.\n"
            "- If a fragment/text contains a quote, escape it with backslash.\n"
            "- The response MUST start with '{{' and end with '}}'.\n\n"
            "If you are unsure about JSON validity, output an empty list:\n"
            "{{ \"annotation_scaffolds\": [] }}\n\n"
            ...
            "7) Fragment uniqueness: do NOT output duplicate fragments. A fragment can appear at most once in annotation_scaffolds.\n\n"
            "8) Ordering: return annotation_scaffolds in document order, based on where each fragment first appears in reading_chunks.\n\n"
            ...
            "  - \"fragment\": exact text from the reading (copied from reading_chunks), capped at <= 5 sentences, and\n"
            "  - \"text\": the generated scaffold (1–3 sentences).\n"
            "- HARD CONSTRAINT: never output a fragment longer than 5 sentences; trim to the most representative <= 5 sentences when needed.\n"
            "- HARD CONSTRAINT: all fragments selected must be unique (no exact duplicates, case-insensitive duplicates, or whitespace-only variations).\n"
            "- HARD CONSTRAINT: each fragment must be a verbatim substring from reading_chunks text; no paraphrase, no rewriting, no normalization, no added/removed words.\n"
            "- HARD CONSTRAINT: output must be sorted by fragment position in the source reading (earlier fragment first).\n"
        ...
        (
            ...
            "- Ensure every returned fragment is <= 5 sentences.\n"
            "- Ensure no two scaffolds use the same fragment.\n"
            "- Ensure every fragment is copied verbatim from reading_chunks.\n"
            "- Order the scaffolds by fragment position in the reading (top-to-bottom / earlier-to-later).\n"
