...
const buildRunClassProfileRequest = (data: ClassProfile, courseId?: string) => ({
  ...
  ...(courseId && courseId !== 'new' ? { course_id: courseId } : {}),
});
...
export default function ViewCoursePage() {
  ...
  useEffect(() => {
    if (!courseId || courseId === 'new') {
      const defaults = createDefaultProfile(courseId || 'new');
...
const createDefaultProfile = (id: string): ClassProfile => ({
  ...
  const loadProfile = async () => {
    ...
    try {
      ...
      // First, fetch course data to get class_profile_id
      const instructorId = urlInstructorId || MOCK_INSTRUCTOR_ID;
      const courseResponse = await fetch(`/api/courses/instructor/${instructorId}`);
      ...
      if (Array.isArray(courseData.courses)) {
        ...
        // If course has a class_profile_id, load the class profile
        if (course.class_profile_id) {
          const profileResponse = await fetch(`/api/class-profiles/${course.class_profile_id}`);
          const profileData = await profileResponse.json().catch(() => ({}));
          
          if (profileResponse.ok) {
            const profilePayload =
              profileData.profile ?? profileData.class_profile ?? profileData.review ?? null;
            ...
            if (loadedProfile) {
              ...
              // No profile data, initialize with defaults
              const defaults = createDefaultProfile(courseId);
              setFormData(cloneProfile(defaults));
              setInitialData(cloneProfile(defaults));
            }
          } else {
            // Failed to load profile, initialize with defaults
            const defaults = createDefaultProfile(courseId);
            setFormData(cloneProfile(defaults));
            setInitialData(cloneProfile(defaults));
          }
        } else {
          // No class profile linked, initialize with defaults
          const defaults = createDefaultProfile(courseId);
          setFormData(cloneProfile(defaults));
          setInitialData(cloneProfile(defaults));
        }
      ...
      setError('Failed to load course or profile. Please try again.');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const handleRegenerate = async (target: RegenerateTarget) => {
    ...
    try {
      const payload = buildRunClassProfileRequest(formData, urlCourseId || undefined);
  ...
  const handleGenerateProfile = async () => {
    ...
    try {
      const payload = buildRunClassProfileRequest(currentData, urlCourseId || undefined);
      ...
      if (updatedFormData.id && updatedFormData.id !== 'new' && courseId === 'new') {
        ...
        router.replace(`/courses/${urlCourseId || courseId}/view${queryString ? `?${queryString}` : ''}`);
      }
    } catch (err: unknown) {
      const message =
  ...
  const handleSave = async (options: SaveOptions = {}) => {
    ...
    try {
      ...
      if (!hasExistingId && savedId && savedId !== 'new' && courseId === 'new') {
        // Preserve course_id and instructor_id when navigating
        const navParams = new URLSearchParams();
        if (urlCourseId) navParams.set('courseId', urlCourseId);
        if (urlInstructorId) navParams.set('instructorId', urlInstructorId);
  ...
  const handleStartSession = () => {
    if (!courseId || courseId === 'new') {
      setError('Course ID is required to start a session.');
      return;
    }
    // Get course_id from URL params
    const resolvedCourseId = urlCourseId || courseId || MOCK_COURSE_ID;
    ...
    router.push(`/courses/${resolvedCourseId}/reading?${params.toString()}`);
  };
