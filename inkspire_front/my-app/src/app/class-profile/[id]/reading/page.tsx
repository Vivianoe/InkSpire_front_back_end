...
export default function ReadingUploadPage() {
  // Path parameters (from route: /class-profile/[id]/reading or /courses/[courseId]/class-profiles/[profileId]/reading)
  ...
  const uploadNewReadings = async (files: FileList) => {
    ...
    try {
      ...
      // course_id is now in the path, not in the payload
      ...
      const response = await fetch(`/api/courses/${courseIdStr}/readings/batch-upload`, {
        method: 'POST',
  ...
  const handleRemoveReading = async (id: string) => {
    ...
    try {
      ...
      const courseIdStr = Array.isArray(resolvedCourseId) ? resolvedCourseId[0] : resolvedCourseId;
      const response = await fetch(`/api/courses/${courseIdStr}/readings/${id}`, {
        method: 'DELETE',
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) {
  ...
  const handleCreateSession = () => {
    ...
    if (courseIdStr && profileIdStr) {
      router.push(`/courses/${courseIdStr}/class-profiles/${profileIdStr}/session/create`);
    } else {
      ...
      if (profileIdStr) params.set('profileId', profileIdStr);
        ...
        <div className={styles.header}>
          <div>
            ...
            <p className={styles.subtitle}>
              Upload and process readings for this class.
            </p>
          </div>
            ...
            <button
              onClick={() => {
                ...
                if (resolvedCourseId && profileId) {
                  router.push(`/courses/${resolvedCourseId}/class-profiles/${profileId}/view`);
                } else {
                  // Fallback to old structure with query params
                  const params = new URLSearchParams();
                  ...
                  router.push(`/class-profile/${profileId}/view${queryString ? `?${queryString}` : ''}`);
                }
              }}
          ...
          <div className={styles.uploadList}>
            {loadingList ? (
              ...
              <div className={styles.emptyState}>
                Add readings for this class to start building your library.
              </div>
            ) : (
